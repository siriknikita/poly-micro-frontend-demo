...
vi.mock('../../components/testing/hooks', () => {
  return {
    useTestItems: vi.fn().mockImplementation((tests: TestItem[], projectId: string, microserviceId: string) => {
...
vi.mock('../../components/testing/components/TestItem', () => ({
  TestItemComponent: ({ item, isExpanded, onToggleExpand }: { item: TestItem, isExpanded: boolean, onToggleExpand: (id: string) => void }) => (
      ...
      {item.children && isExpanded && (
        <div data-testid={`children-${item.id}`}>
          {item.children.map(child => (
            <div key={child.id} data-testid={`child-${child.id}`}>{child.name}</div>
          ))}
        </div>
      )}
    </div>
...
vi.mock('../../components/testing/components/TestOutputModal', () => ({
  TestOutputModal: ({ isOpen }: { isOpen: boolean }) => isOpen ? <div data-testid="test-output-modal"></div> : null,
}));
