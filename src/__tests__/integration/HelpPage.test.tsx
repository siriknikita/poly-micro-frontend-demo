...
describe('HelpPage Integration', () => {
  ...
  it('allows users to search for FAQs and view answers', async () => {
    ...
    expect(screen.getByText(/Navigate to the Microservices tab, click the "Add Service" button/)).toBeInTheDocument();
  });
  ...
  it('allows users to submit a question when FAQs don\'t help', async () => {
    ...
    await user.type(screen.getByTestId('question-text-input'), 'I would like to request a feature to export all microservice logs at once.');
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Your question has been submitted successfully. We\'ll get back to you as soon as possible.')).toBeInTheDocument();
    });
    
    // Question should be logged
    expect(console.log).toHaveBeenCalledWith('Question submitted:', expect.objectContaining({
      name: 'Test User',
  ...
  it('shows no results and ask question form when search has no matches', async () => {
    ...
    expect(screen.getByText('We couldn\'t find any FAQs matching your search. Try a different search term or ask your question below.')).toBeInTheDocument();
