...
export const Dropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <div className={`relative inline-block text-left ${className}`} ref={dropdownRef} data-testid={testId}>
      <button
        type="button"
        ...
        >
          {header && (
            <div className="px-4 py-3">
              {header.title && <p className="text-sm leading-5 text-gray-500 dark:text-gray-400">{header.title}</p>}
              {header.subtitle && <p className="text-sm font-medium leading-5 text-gray-900 dark:text-gray-100 truncate">{header.subtitle}</p>}
            </div>
          )}
