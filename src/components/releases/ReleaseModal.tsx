...
const ReleaseModal: React.FC = () => {
  ...
  const renderReleaseCard = (release: typeof latestRelease, isLatest: boolean = false) => {
    ...
    return (
      <div 
        ...
        className={`border rounded-md p-4 mb-3 ${isLatest 
          ? darkMode 
      ...
      >
        <div className="flex justify-between items-center">
          <div className="flex items-center gap-2">
            ...
            <span className={`${darkMode ? 'bg-blue-900 bg-opacity-30 text-blue-300' : 'bg-blue-100 text-blue-800'} text-xs px-2 py-1 rounded-full`}>
              v{release.version}
            </span>
            {isLatest && (
              <span className={`${darkMode ? 'bg-green-900 bg-opacity-30 text-green-300' : 'bg-green-100 text-green-800'} text-xs px-2 py-1 rounded-full`}>
                Latest
              </span>
            )}
          </div>
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className={`${darkMode ? 'bg-gray-800 text-gray-100' : 'bg-white text-gray-800'} rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col`}>
        ...
        <div className={`p-4 ${darkMode ? 'border-gray-700' : 'border-gray-200'} border-t flex justify-end`}>
          <button
