...
const ReleaseDebug: React.FC = () => {
  ...
  const acknowledgments = useLiveQuery(async () => {
              ...
              <ul className="text-sm space-y-2">
                {releases.map((release, index) => (
                  <li key={index} className="border-b pb-2">
                    <div><strong>Version:</strong> {release.version}</div>
                    <div><strong>Title:</strong> {release.title}</div>
                    <div><strong>isLatest:</strong> {release.isLatest === 1 ? 'Yes' : 'No'}</div>
                    <div><strong>ID:</strong> {release.id ? String(release.id) : 'undefined'}</div>
                    <div><strong>Date:</strong> {release.releaseDate.toLocaleString()}</div>
                    <div><strong>Changes:</strong> {release.changes.length}</div>
                  </li>
                ))}
              </ul>
            )}
            ...
            {acknowledgments.length === 0 ? (
              ...
              <ul className="text-sm">
                {acknowledgments.map((ack, index) => (
                  <li key={index}>
                    User {String(ack.userId)} acknowledged release {String(ack.releaseId)} at {ack.acknowledgedAt.toLocaleString()}
                  </li>
                ))}
              </ul>
            )}
