...
export const IconButton = ({
  ...
  const baseClasses = 'flex items-center justify-center rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 dark:focus:ring-offset-gray-900';
  ...
  return (
    ...
    >
      {icon}
      {label && <span className={`${size === 'sm' ? 'ml-1' : 'ml-2'} ${size === 'sm' ? 'text-xs' : 'text-sm'}`}>{label}</span>}
    </button>
  );
};
