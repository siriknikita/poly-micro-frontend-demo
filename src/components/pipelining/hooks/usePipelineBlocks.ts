...
export function usePipelineBlocks(options: UsePipelineBlocksOptions = {}) {
  ...
  const addBlock = useCallback((blockData: PipelineBlock, position: { x: number, y: number }) => {
  ...
  const moveBlock = useCallback((instanceId: string, newPosition: { x: number; y: number }) => {
