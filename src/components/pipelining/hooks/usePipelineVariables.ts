...
export function usePipelineVariables({ initialVariables = [] }: UsePipelineVariablesOptions = {}) {
  const [variables, setVariables] = useState<PipelineVariable[]>(initialVariables.length > 0 
    ? initialVariables 
    : [
  ...
  const addVariable = useCallback((variable?: PipelineVariable) => {
  ...
  const updateVariable = useCallback((index: number, field: keyof PipelineVariable, value: string) => {
